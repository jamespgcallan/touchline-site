...
/* Article sharing: add a consistent share control at the top and bottom of every article page. */
document.addEventListener('DOMContentLoaded', () => {
  const articleHead = document.querySelector('.article-head');
  const articleBody = document.querySelector('.article-body');
  if (!articleHead || !articleBody) return;

  const shareStyle = document.createElement('style');
  shareStyle.textContent = `
    .article-share-row{display:flex;align-items:center;margin-top:18px}
    .article-share-row.bottom{margin:42px 0 8px;padding-top:22px;border-top:1px solid var(--line,rgba(33,31,26,.10))}
    .article-share-btn{display:inline-flex;align-items:center;gap:8px;border:1px solid var(--line,rgba(33,31,26,.10));background:transparent;color:var(--ink,#1c1a17);padding:9px 14px;border-radius:999px;font:600 12.5px 'Inter',sans-serif;cursor:pointer;transition:background .2s,color .2s,border-color .2s,transform .15s ease}
    .article-share-btn:hover,.article-share-btn:focus-visible{background:var(--accent-soft,#fbe2c4);color:var(--accent,#e8720f);border-color:rgba(232,114,15,.25);transform:translateY(-1px);outline:none}
    .article-share-btn svg{width:16px;height:16px;display:block;stroke:currentColor}
    .article-share-btn.is-copied{background:var(--ink,#1c1a17);color:var(--paper,#fbf3e7);border-color:var(--ink,#1c1a17)}
    @media(max-width:600px){.article-share-row{margin-top:16px}.article-share-row.bottom{margin-top:34px}.article-share-btn{padding:9px 13px}}
  `;
  document.head.appendChild(shareStyle);

  const title = articleHead.querySelector('h1')?.textContent.trim() || document.title;
  const text = articleHead.querySelector('.article-dek')?.textContent.trim() || '';
  const url = window.location.href;
  const icon = '<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 16V4m0 0L7.5 8.5M12 4l4.5 4.5M5 13v5a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-5" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  ...
  const makeShareRow = (position) => {
    const row = document.createElement('div');
    row.className = `article-share-row ${position}`;
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'article-share-btn';
    button.setAttribute('aria-label', 'Share this article');
    button.innerHTML = `${icon}<span class="share-label">Share</span>`;
    button.addEventListener('click', () => share(button));
    row.appendChild(button);
    return row;
  };

  const meta = articleHead.querySelector('.article-meta');
  if (meta) meta.insertAdjacentElement('afterend', makeShareRow('top'));
  else articleHead.appendChild(makeShareRow('top'));

  const cta = articleBody.querySelector('.cta-box');
  const bottomShare = makeShareRow('bottom');
  if (cta) articleBody.insertBefore(bottomShare, cta);
  else articleBody.appendChild(bottomShare);
});
