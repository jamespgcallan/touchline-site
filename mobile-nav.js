document.addEventListener('DOMContentLoaded', () => {
  ...
  style.textContent = `
    ...
    @media(max-width:600px){
      ...
      .mobile-menu-btn{display:inline-flex;align-items:center;gap:9px;border:1px solid rgba(33,31,26,.12);background:#fff;color:#1c1a17;border-radius:999px;padding:9px 14px;font:600 13px 'Inter',sans-serif;cursor:pointer;margin-left:auto;position:relative;z-index:102}
      ...
      body.mobile-menu-open::before{content:"";position:fixed;inset:0;background:rgba(28,26,23,.28);backdrop-filter:blur(3px);-webkit-backdrop-filter:blur(3px);z-index:90}
      .mobile-menu-panel{position:absolute;display:none;grid-template-columns:repeat(2,minmax(0,1fr));gap:4px;top:calc(100% + 10px);left:0;right:0;max-height:calc(100vh - 105px);overflow-y:auto;-webkit-overflow-scrolling:touch;background:#fbf3e7;border:1px solid rgba(33,31,26,.10);border-radius:16px;padding:8px;box-shadow:0 18px 40px rgba(33,31,26,.18);z-index:101}
  ...
  sub.href = 'index.html#subscribe';
  sub.className = 'mobile-subscribe';
